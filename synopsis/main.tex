...
\begin{table}[h!]
    ...
    \begin{tabularx}{\textwidth}{lX}
        ...
        \textbf{Languages \& Frameworks} & Python 3.14, LangGraph, LangChain \\
        \bottomrule
    \end{tabularx}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[x=1.5cm, y=1.2cm, >=stealth, scale=0.9, every node/.style={transform shape}]
        ...
        % System boundary
        % -------------------------------------------------------------
        \draw[thick, fill=gray!3] (-2.9, -3.6) rectangle (2.9, 3.6);
        \node at (0, 3.25) [font=\large\bfseries] {Self-Correcting RAG System};

        % -------------------------------------------------------------
        % Use Cases
        % -------------------------------------------------------------
        \node (uc1) [draw, ellipse, minimum width=2.6cm, minimum height=0.8cm, fill=orange!10, draw=orange!80, thick, align=center] at (0, 2.2)  {\small Submit Query};
        \node (uc2) [draw, ellipse, minimum width=2.6cm, minimum height=0.8cm, fill=orange!10, draw=orange!80, thick, align=center] at (0, 1.1)  {\small View Response};
        \node (uc3) [draw, ellipse, minimum width=2.6cm, minimum height=0.8cm, fill=orange!10, draw=orange!80, thick, align=center] at (0, 0.0)  {\small Browse Sources};
        \node (uc4) [draw, ellipse, minimum width=2.6cm, minimum height=0.8cm, fill=orange!10, draw=orange!80, thick, align=center] at (0, -1.1) {\small Manage Corpus};
        \node (uc5) [draw, ellipse, minimum width=2.6cm, minimum height=0.8cm, fill=orange!10, draw=orange!80, thick, align=center] at (0, -2.2) {\small Configure System};

        % -------------------------------------------------------------
        % Left Actor: User (blue accent, matches DFD entity color)
        % -------------------------------------------------------------
        \begin{scope}[shift={(-4.3, 1.5)}]
            \draw[thick, blue!70] (0,0.4) circle (0.15);
            \draw[thick, blue!70] (0,0.25) -- (0,-0.2);
            \draw[thick, blue!70] (-0.25,0.1) -- (0.25,0.1);
            \draw[thick, blue!70] (0,-0.2) -- (-0.15,-0.6);
            \draw[thick, blue!70] (0,-0.2) -- (0.15,-0.6);
        ...
        % -------------------------------------------------------------
        % Right Actor: Admin / Orchestrator (green accent)
        % -------------------------------------------------------------
        \begin{scope}[shift={(4.3, -0.1)}]
            \draw[thick, green!50!black] (0,0.4) circle (0.15);
            \draw[thick, green!50!black] (0,0.25) -- (0,-0.2);
            \draw[thick, green!50!black] (-0.25,0.1) -- (0.25,0.1);
            \draw[thick, green!50!black] (0,-0.2) -- (-0.15,-0.6);
            \draw[thick, green!50!black] (0,-0.2) -- (0.15,-0.6);
            \node at (0,-0.85) [font=\bfseries] {Orchestrator};
            \coordinate (admin) at (-0.25, 0.1);
        \end{scope}

        % -------------------------------------------------------------
        % User connections — comb rail at x = -2.35 (inside boundary,
        % clear of ellipse west edges at x = -1.3)
        % -------------------------------------------------------------
        \draw[blue!60] (user) -- (-2.35, 1.6);           % actor stub to spine
        \draw[blue!60] (-2.35, 2.2) -- (-2.35, 1.1);      % vertical spine
        \draw[blue!60] (-2.35, 2.2) -- (uc1.west);        % spur
        \draw[blue!60] (-2.35, 1.1) -- (uc2.west);        % spur

        % -------------------------------------------------------------
        % Admin connections — comb rail at x = 2.35 (inside boundary,
        % clear of ellipse east edges at x = 1.3)
        % -------------------------------------------------------------
        \draw[green!50!black] (admin) -- (2.35, 0.0);              % actor stub to spine
        \draw[green!50!black] (2.35, 2.2) -- (2.35, -2.2);          % vertical spine
        \draw[green!50!black] (2.35, 2.2)  -- (uc1.east);           % spur
        \draw[green!50!black] (2.35, 0.0)  -- (uc3.east);           % spur
        \draw[green!50!black] (2.35, -1.1) -- (uc4.east);           % spur
        \draw[green!50!black] (2.35, -2.2) -- (uc5.east);           % spur
...
\begin{figure}[htbp]
    \centering
    \begin{tikzpicture}[x=2.2cm, y=-1.0cm, >=stealth, scale=0.85, every node/.style={transform shape}]
        
        % -------------------------------------------------------------
        % 1. STAGGERED PARTICIPANTS (Alternating Y-positions)
        % -------------------------------------------------------------
        % Top Row (y = -0.6)
        \node (user)   at (0,-0.4) [draw, rounded corners=3pt, fill=blue!15,   minimum width=1.8cm, minimum height=0.7cm, font=\bfseries\small] {User};
        \node (ret)    at (2,-0.4) [draw, rounded corners=3pt, fill=green!15,  minimum width=1.8cm, minimum height=0.7cm, font=\bfseries\small] {Retriever};
        \node (search) at (4,-0.4) [draw, rounded corners=3pt, fill=purple!15, minimum width=1.8cm, minimum height=0.7cm, font=\bfseries\small] {Search API};
        \node (crit)   at (6,-0.4) [draw, rounded corners=3pt, fill=cyan!15,   minimum width=1.8cm, minimum height=0.7cm, font=\bfseries\small] {Critique};

        % Bottom Row (y = 0.6) - Staggered to prevent adjacent box collisions
        \node (orch)   at (1,0.4)  [draw, rounded corners=3pt, fill=orange!15, minimum width=1.8cm, minimum height=0.7cm, font=\bfseries\small] {Orchestrator};
        \node (eval)   at (3,0.4)  [draw, rounded corners=3pt, fill=yellow!20, minimum width=1.8cm, minimum height=0.7cm, font=\bfseries\small] {Evaluator};
        \node (gen)    at (5,0.4)  [draw, rounded corners=3pt, fill=red!15,    minimum width=1.8cm, minimum height=0.7cm, font=\bfseries\small] {Generator};

        % Stagger Connection Indicators (Visual linking from top row nodes down to baseline)
        \draw[dashed, gray] (user)   -- (0,1.2);
        \draw[dashed, gray] (ret)    -- (2,1.2);
        \draw[dashed, gray] (search) -- (4,1.2);
        \draw[dashed, gray] (crit)   -- (6,1.2);
        \draw[dashed, gray] (orch)   -- (1,1.2);
        \draw[dashed, gray] (eval)   -- (3,1.2);
        \draw[dashed, gray] (gen)    -- (5,1.2);

        % -------------------------------------------------------------
        % 2. LIFELINES
        % -------------------------------------------------------------
        \draw[dashed, gray] (0,1.2) -- (0,16.5);
        \draw[dashed, gray] (1,1.2) -- (1,16.5);
        \draw[dashed, gray] (2,1.2) -- (2,16.5);
        \draw[dashed, gray] (3,1.2) -- (3,16.5);
        \draw[dashed, gray] (4,1.2) -- (4,16.5);
        \draw[dashed, gray] (5,1.2) -- (5,16.5);
        \draw[dashed, gray] (6,1.2) -- (6,16.5);

        % Main Orchestrator Activation Bar
        \draw[fill=white, draw=black] (0.95,2.0) rectangle (1.05,15.8);

        % -------------------------------------------------------------
        % 3. INITIAL RETRIEVAL & EVALUATION
        % -------------------------------------------------------------
        % 1. Submit Query
        \draw[->, thick] (0,2.2) -- node[above, scale=0.7] {1. Submit Query} (1,2.2);

        % 2. Retrieve Docs
        \draw[->] (1,3.0) -- node[above, scale=0.7] {2. Retrieve(Query)} (2,3.0);
        \draw[fill=white] (1.95,3.0) rectangle (2.05,3.8);
        \draw[<-, dashed] (1,3.8) -- node[above, scale=0.7] {Internal Docs} (2,3.8);

        % 3. Grade Relevance
        \draw[->] (1,4.6) -- node[above, scale=0.7] {3. Grade Relevance(Docs)} (3,4.6);
        \draw[fill=white] (2.95,4.6) rectangle (3.05,5.4);
        \draw[<-, dashed] (1,5.4) -- node[above, scale=0.7] {Relevance Score} (3,5.4);

        % -------------------------------------------------------------
        % 4. ALT BLOCK: WEB SEARCH FALLBACK
        % -------------------------------------------------------------
        \draw[thick, fill=gray!5, fill opacity=0.4] (0.4,5.9) rectangle (4.4,8.3);
        \node[anchor=north west, scale=0.65, font=\bfseries] at (0.4,5.9) {alt [Doc Relevance < Threshold]};
        
        \draw[->] (1,6.7) -- node[above, scale=0.7] {4. Search Web(Query)} (4,6.7);
        \draw[fill=white] (3.95,6.7) rectangle (4.05,7.5);
        \draw[<-, dashed] (1,7.5) -- node[above, scale=0.7] {Web Docs} (4,7.5);

        % -------------------------------------------------------------
        % 5. INITIAL GENERATION
        % -------------------------------------------------------------
        \draw[->] (1,8.9) -- node[above, scale=0.7] {5. Generate(Filtered Context)} (5,8.9);
        \draw[fill=white] (4.95,8.9) rectangle (5.05,9.7);
        \draw[<-, dashed] (1,9.7) -- node[above, scale=0.7] {Draft Answer} (5,9.7);

        % -------------------------------------------------------------
        % 6. CRITIQUE & SELF-CORRECTION LOOP
        % -------------------------------------------------------------
        \draw[->] (1,10.4) -- node[above, scale=0.7] {6. Evaluate Answer(Draft, Context)} (6,10.4);
        \draw[fill=white] (5.95,10.4) rectangle (6.05,11.2);
        \draw[<-, dashed] (1,11.2) -- node[above, scale=0.7] {Critique / Score} (6,11.2);

        % Loop Block
        \draw[thick, fill=orange!5, fill opacity=0.4] (0.4,11.7) rectangle (6.4,15.1);
        \node[anchor=north west, scale=0.65, font=\bfseries] at (0.4,11.7) {loop [Until Score Meets Criteria or Max Retries]};

        % Refinement
        \draw[->] (1,12.6) -- node[above, scale=0.7] {7a. Refine Answer(Feedback)} (5,12.6);
        \draw[fill=white] (4.95,12.6) rectangle (5.05,13.4);
        \draw[<-, dashed] (1,13.4) -- node[above, scale=0.7] {Refined Answer} (5,13.4);

        % Re-evaluate
        \draw[->] (1,14.1) -- node[above, scale=0.7] {7b. Re-evaluate(Refined Answer)} (6,14.1);
        \draw[fill=white] (5.95,14.1) rectangle (6.05,14.7);
        \draw[<-, dashed] (1,14.7) -- node[above, scale=0.7] {Updated Score} (6,14.7);

        % -------------------------------------------------------------
        % 7. RETURN ANSWER
        % -------------------------------------------------------------
        \draw[->, thick] (1,15.5) -- node[above, scale=0.7] {8. Return Verified Answer} (0,15.5);

    \end{tikzpicture}
    \caption{Self-Corrective RAG Sequence Diagram}
    \label{fig:sequence}
\end{figure}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[node distance=2.5cm, >=stealth, scale=0.9, every node/.style={transform shape}]
        ...
        \node (system) [draw, circle, minimum width=3cm, minimum height=3cm, fill=orange!5, align=center] at (5.0, 0) {Self-Correcting\\RAG System};
        \node (admin) [draw, rectangle, minimum width=2cm, minimum height=1cm, fill=green!5] at (10.0, 0) {Administrator};
        
        % Flows
        \draw[->, thick] (user.23) -- node[above, scale=0.75] {Query} (system.165);
        \draw[<-, thick] (user.337) -- node[below, scale=0.75] {Response} (system.195);
        
        \draw[->, thick] (admin.163) -- node[above, scale=0.75] {Corpus / Config} (system.15);
        \draw[<-, thick] (admin.197) -- node[below, scale=0.75] {Status / Logs} (system.345);
    \end{tikzpicture}
...
\begin{figure}[h!]
    \centering
    \begin{tikzpicture}[>=stealth,scale = 0.935, every node/.style={transform shape}]

        % -------------------------------------------------------------
        % 1. EXTERNAL ENTITIES
        % -------------------------------------------------------------
        \node (user)  at (0, 4)    [draw, rectangle, rounded corners=2pt, fill=blue!10, minimum width=1.7cm, minimum height=0.9cm, font=\bfseries\small] {User};
        \node (admin) at (3.3, -1.3) [draw, rectangle, rounded corners=2pt, fill=green!10, minimum width=1.7cm, minimum height=0.9cm, font=\bfseries\small] {Admin};
        \node (web)   at (8.3,-1.3)[draw, rectangle, rounded corners=2pt, fill=purple!10, minimum width=1.9cm, minimum height=0.9cm, font=\bfseries\small] {Web Search API};

        % -------------------------------------------------------------
        % 2. PROCESSES (main pipeline row, y = 4)
        % -------------------------------------------------------------
        \node (p1) at (3.3, 4)  [draw, circle, fill=orange!10, minimum size=1.6cm, align=center, font=\small] {\textbf{1.0}\\\scriptsize Retrieve};
        \node (p2) at (6.6, 4)  [draw, circle, fill=orange!10, minimum size=1.6cm, align=center, font=\small] {\textbf{2.0}\\\scriptsize Evaluate};
        \node (p3) at (8.3,1.3)[draw, circle, fill=orange!10, minimum size=1.6cm, align=center, font=\small] {\textbf{3.0}\\\scriptsize Fallback Search};
        \node (p4) at (9.9, 4)  [draw, circle, fill=orange!10, minimum size=1.6cm, align=center, font=\small] {\textbf{4.0}\\\scriptsize Generate};
        \node (p5) at (13.2,4)  [draw, circle, fill=orange!10, minimum size=1.6cm, align=center, font=\small] {\textbf{5.0}\\\scriptsize Critique};

        % -------------------------------------------------------------
        % 3. DATA STORES (open-ended notation: top/bottom rules only)
        % -------------------------------------------------------------
        \node (ds1) at (3.3, 1.3) [fill=yellow!15, minimum width=2.1cm, minimum height=0.6cm, align=left, font=\scriptsize] {\textbf{D1}\quad Vector Database};
        \draw[thick] (ds1.north west) -- (ds1.north east);
        \draw[thick] (ds1.south west) -- (ds1.south east);

        \node (ds2) at (13.2,1.3) [fill=yellow!15, minimum width=2.1cm, minimum height=0.6cm, align=left, font=\scriptsize] {\textbf{D2}\quad State Store};
        \draw[thick] (ds2.north west) -- (ds2.north east);
        \draw[thick] (ds2.south west) -- (ds2.south east);

        % -------------------------------------------------------------
        % 4. DATA FLOWS
        % -------------------------------------------------------------
        % User & Admin entry flows
        \draw[->, thick] (user)  -- node[above, scale=0.7] {User Query} (p1);
        \draw[->] (admin) -- node[right, scale=0.7] {Source Docs} (ds1);

        % P1 <-> D1 (offset so the two directions never overlap)
        \draw[->] ($(p1.south)+(-0.15,0)$) -- node[left,  scale=0.65] {Query Vector}     ($(ds1.north)+(-0.15,0)$);
        \draw[<-] ($(p1.south)+( 0.15,0)$) -- node[right, scale=0.65] {Retrieved Chunks} ($(ds1.north)+( 0.15,0)$);

        % Main sequence
        \draw[->] (p1) -- node[above, scale=0.7] {Context} (p2);
        \draw[->] (p2) -- node[above, scale=0.7] {Valid Docs} (p4);

        % Fallback search branch
        \draw[->] (p2) -- node[left, scale=0.65, pos=0.55] {Low Relevance} (p3);
        \draw[->] (p3) -- node[left, scale=0.65] {Search Query} (web);
        \draw[<-] ($(p3.south)+(0.35,0)$) -- node[right, scale=0.65] {Search Results} ($(web.north)+(0.35,0)$);
        \draw[->] (p3) -- node[right, scale=0.65, pos=0.55] {Augmented Context} (p4);

        % Generation -> Critique -> Log
        \draw[->, thick] (p4) -- node[above, scale=0.7] {Draft Answer} (p5);
        \draw[->] (p5) -- node[right, scale=0.7] {Log State/Score} (ds2);

        % Refine loop: dashed arc dips BELOW the row, clear of P3 and D2
        \draw[->, dashed] (p5.south) to[out=250, in=290, distance=1.4cm]
            node[below, scale=0.65] {Refine / Correct} (p4.south);

        % Final response: dedicated lane ABOVE the row, clear of every node
        \draw[->, thick] (p5.north) |- ++(0,1.5) -| node[above, pos=0.5, scale=0.7] {Final Verified Response} (user.north);

    \end{tikzpicture}
    \caption{Level 1 Data Flow Diagram (DFD) for Self-Corrective RAG.}
    \label{fig:dfd1}
\end{figure}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[x=0.62cm, y=-0.65cm, >=stealth, scale=0.9, every node/.style={transform shape}]
        ...
        % -------------------------------------------------------------
        % LAYOUT CONSTANTS
        % Label column: x = -4 to 4   (wide, for long phase names)
        % Timeline:     x =  4 to 20  (16 weeks, 1 unit each)
        % Header row:   y =  0 to 1
        % Phase rows:   y =  1 to 7   (6 rows, 1 unit each)
        % -------------------------------------------------------------

        % Timeline grid (only in the timeline region, aligned to week boundaries)
        \draw[gray!25, very thin] (4,0) grid (20,7);

        % Divider between label column and timeline
        \draw[black, thick] (4,0) -- (4,7);
        % Divider between header row and phase rows
        \draw[black, thick] (-4,1) -- (20,1);
        % Outer frame
        \draw[black, thick] (-4,0) rectangle (20,7);

        % Week Headers (centered in each week cell)
        \foreach \w in {1,...,16} {
            \node at (4+\w-0.5, 0.5) [scale=0.65, font=\bfseries] {W\w};
        }
        \node at (0, 0.5) [scale=0.65, font=\bfseries] {Phase / Task};

        % Phase Labels (left-aligned, generous room in the wide label column)
        \node at (-3.8, 1.5) [scale=0.65, align=left, anchor=west] {P1: Lit Survey \& Design};
        \node at (-3.8, 2.5) [scale=0.65, align=left, anchor=west] {P2: Ingestion \& Vector DB};
        \node at (-3.8, 3.5) [scale=0.65, align=left, anchor=west] {P3: CRAG Evaluator};
        \node at (-3.8, 4.5) [scale=0.65, align=left, anchor=west] {P4: Web Search Integration};
        \node at (-3.8, 5.5) [scale=0.65, align=left, anchor=west] {P5: Self-Reflection \& LangGraph};
        \node at (-3.8, 6.5) [scale=0.65, align=left, anchor=west] {P6: Integration \& Thesis};

        % Gantt Bars (each snapped exactly to its week-cell boundaries)
        \draw[fill=blue!20,   draw=blue!80,   thick] (4+0,  1.25) rectangle (4+3,  1.75);  % P1: W1-3
        \draw[fill=orange!20, draw=orange!80, thick] (4+3,  2.25) rectangle (4+6,  2.75);  % P2: W4-6
        \draw[fill=green!20,  draw=green!80,  thick] (4+6,  3.25) rectangle (4+9,  3.75);  % P3: W7-9
        \draw[fill=yellow!20, draw=yellow!80, thick] (4+9,  4.25) rectangle (4+12, 4.75);  % P4: W10-12
        \draw[fill=purple!20, draw=purple!80, thick] (4+12, 5.25) rectangle (4+14, 5.75);  % P5: W13-14
        \draw[fill=red!20,    draw=red!80,    thick] (4+14, 6.25) rectangle (4+16, 6.75);  % P6: W15-16
